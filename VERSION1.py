...
init_db()
class SonnerToast:
    def __init__(self, root, message, buttons=None,progressbar=None):
        self.root = root
        ...
        self.toast.geometry("340x60" if not self.buttons else "340x110")
        self.toast.attributes("-alpha", 0.0)
        self.toast.attributes("-topmost", True)

        screen_width = root.winfo_screenwidth()
        screen_height = root.winfo_screenheight()
        self.toast.geometry(f"+{screen_width - 360}+{screen_height - (140 if self.buttons else 100)}")

        bg_color = "#FFFFFF"
        text_color = "#000000"

        self.canvas = tk.Canvas(self.toast, bg=bg_color, highlightthickness=0, bd=0)
        self.canvas.pack(fill="both", expand=True)

        # More rounded (20px)
        self.canvas.create_rounded_rect(0, 0, 340, 110 if self.buttons else 60, 20, fill=bg_color, outline="")

        self.label = tk.Label(self.canvas, text=message, fg=text_color, bg=bg_color, font=("Segoe UI", 10), padx=20, pady=10)
        self.canvas.create_window(170, 25, window=self.label)

        if self.buttons:
            btn_frame = tk.Frame(self.canvas, bg=bg_color)
            for i, btn in enumerate(self.buttons):
                b = tk.Button(
                    btn_frame, text=btn["text"],
                    command=lambda cmd=btn["command"]: self._on_button(cmd),
                    relief="flat", bg="#4e73df", fg="white",
                    font=("Segoe UI", 9, "bold"), padx=10, pady=2, borderwidth=0
                )
                b.pack(side="left", padx=8)
            self.canvas.create_window(170, 80, window=btn_frame)
        self.fade_in()
        if not self.buttons:
...
class DocumentQnAApp:
    ...
    def show_main_app(self):
        ...
        try:
            user_img = Image.open("user.png").resize((80, 80), Image.LANCZOS)
    ...
    def show_settings_screen(self):
        ...
        try:
            profile_img = Image.open("user.png").resize((150, 150), Image.LANCZOS)
            self.profile_img = ImageTk.PhotoImage(profile_img)
            ttk.Label(profile_container, image=self.profile_img).pack(padx=10, pady=10)
        except:
            ttk.Label(profile_container, text="👤", font=('Helvetica', 72),
                      padx=10, pady=10).pack()

        ttk.Button(left_frame, text="Change Picture", command=self.change_profile_picture,
    ...
    def show_history_screen(self):
        ...
        def on_delete_chat():
            ...
            def on_no():
                pass
            SonnerToast(self.root, "Are you sure you want to delete this chat?", buttons=[{"text": "Yes", "command": on_yes}, {"text": "No", "command": on_no}])
    ...
    def handle_login(self):
        ...
        if success:
            ...
            if result:
                ...
                self.show_main_app()
                SonnerToast(self.root, "Welcome Back!",
                            buttons=[{"text": "OK", "command": None}])
            else:
    ...
    def change_profile_picture(self):
        ...
        if filepath:
            try:
                ...
                profile_img = Image.open(filepath).resize((100, 100), Image.LANCZOS)
                self.profile_img = ImageTk.PhotoImage(profile_img)
                # Update the displayed image
                self.content_area.winfo_children()[0].winfo_children()[0].winfo_children()[1].config(
                    image=self.profile_img)
            except Exception as e:
                SonnerToast(self.root, "Failed to update profile picture: " + str(e),
                            buttons=[{"text": "OK", "command": None}])
    ...
    def new_chat(self):
        ...
        if self.current_chat:
            ...
            def on_no():
                ...
                self.current_highlighted_source = None
                self.show_chat_screen()
            SonnerToast(self.root, "Do you want to save the current chat before starting a new one?", buttons=[{"text": "Yes", "command": on_yes}, {"text": "No", "command": on_no}])
        else:
            self.current_chat = []
            self.current_chat_id = None
            self.current_highlighted_source = None
    ...
    # def open_document(self, doc_id, filepath):
    #     if not os.path.exists(filepath):
    #         SonnerToast(self.root, "The document file was not found at the original location!",
    #                     buttons=[{"text": "OK", "command": None}])
    #         return
    ...
    #     self.current_document_id = doc_id
    #
    #     # Don't try to update file_label here
    #     # The document name will be shown in the chat screen header
    #
